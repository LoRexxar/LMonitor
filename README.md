...
- Django 配置：[settings.py](file:///d:/program/LMonitor/LMonitor/settings.py)（可选本地覆盖：[settings_local.py.example](file:///d:/program/LMonitor/LMonitor/settings_local.py.example)）
- 路由定义：[urls.py](file:///d:/program/LMonitor/LMonitor/urls.py)

## 2. 目录结构（关键部分）

- `LMonitor/`：Django project 配置层
  - [settings.py](file:///d:/program/LMonitor/LMonitor/settings.py)：数据库、静态资源、外部服务配置、线程配置（会尝试导入 `settings_local.py` 做覆盖）
  - [settings_local.py.example](file:///d:/program/LMonitor/LMonitor/settings_local.py.example)：本地覆盖示例（通常只放数据库等环境差异配置）
...
- Python：建议 Python 3.8+（项目最初基于 Django 3.0.1，见 [settings.py](file:///d:/program/LMonitor/LMonitor/settings.py#L1-L10)）
- 数据库：默认 MySQL；本地快速验证可切到 SQLite（见下文）
- 配置覆盖：项目会尝试导入 `LMonitor/settings_local.py` 覆盖配置（参考 [settings_local.py.example](file:///d:/program/LMonitor/LMonitor/settings_local.py.example)）
...
4. 启动 Web：`python manage.py runserver 0.0.0.0:18000`（见 [webstart.sh](file:///d:/program/LMonitor/webstart.sh)）
5. 启动后台：`python manage.py LMonitorCoreBackend`（见 [start.sh](file:///d:/program/LMonitor/start.sh)、[winstart.ps1](file:///d:/program/LMonitor/winstart.ps1)）

### 7.3 MySQL 启动（默认模式）

1. 准备数据库：创建 `LMonitor`（或自定义库名），并确保账号/权限可用
2. 配置连接：在 `LMonitor/settings_local.py` 中覆盖 `DATABASES`（示例见 [settings_local.py.example](file:///d:/program/LMonitor/LMonitor/settings_local.py.example)）
...
- `python manage.py SyncPortalToolsFromKuangBaoZhan`、`python manage.py SyncPortalFavicons`：同步 Portal 工具与图标
- `python manage.py GewechatInit`：gewechat 初始化

### 7.5 功能入口与访问路径

路由汇总见 [urls.py](file:///d:/program/LMonitor/LMonitor/urls.py)：
...
- 项目结构说明（本文档）：[README.md](file:///d:/program/LMonitor/README.md)
- SimC 专项研发记录：
  - 规划：[task_plan.md](file:///d:/program/LMonitor/task_plan.md)
  - 过程记录：[progress.md](file:///d:/program/LMonitor/progress.md)
  - 阶段结论：[findings.md](file:///d:/program/LMonitor/findings.md)
- 本地预览：`preview/`（包含静态预览 HTML 与 [serve_preview.ps1](file:///d:/program/LMonitor/preview/serve_preview.ps1)）
