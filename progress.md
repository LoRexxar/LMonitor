...
### Session 1
- 约定后续页面优化均以“本地 settings 已配置的真实数据库环境”为准（MySQL，数据非空），不再使用 SQLite 空库作为默认验证环境。
- 本地 Django 可用端口：`http://127.0.0.1:8011/`（真实 Portal 首页在根路径 `/`，Dashboard 在 `/dashboard/`）。
- 预览页 `preview/*.html` 仅用于快速视觉示意，不作为“功能完整性/入口完整性”的验收依据；验收以真实 Django 页面为准。
- UI 方向：Dashboard 保持浅色后台风格且不减少入口；Portal 也倾向浅色卡片体系，与后台保持一致。
