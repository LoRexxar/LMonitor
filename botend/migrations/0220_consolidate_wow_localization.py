...
def validate_name(data):
    ...
    if row['kind'] not in ('spell', 'talent', 'item', 'phrase', 'macro'):
...
def ensure_idle(registry, schema_editor):
    leases = registry.get_model('botend', 'MonitorTaskLease').objects.using(schema_editor.connection.alias)
    if leases.filter(task__name='MaxrollClassGuideMonitor', expires_at__gt=timezone.now()).exists():
        raise RuntimeError('请先停止攻略同步，待执行锁释放后再迁移名称资料。')


def move_names(registry, schema_editor):
    using = schema_editor.connection.alias
    old = registry.get_model('botend', 'ClassGuideTerm')
    for row in old.objects.using(using).values(*FIELDS).iterator():
        write_name(row, registry=registry, using=using)


def restore_names(registry, schema_editor):
    using = schema_editor.connection.alias
    old = registry.get_model('botend', 'ClassGuideTerm')
    node = registry.get_model('botend', 'WowTalentNodeMetadata')
    versions = registry.get_model('botend', 'WowTalentVersion').objects.using(using)
    for version in versions:
        label = version_label(version, version.key)
        for row in effective_names(label, registry=registry, using=using):
            old.objects.using(using).get_or_create(game_version=label, kind=row['kind'], object_id=row['object_id'],
                defaults={k: row[k] for k in ('name_en', 'name_zh', 'icon', 'evidence')})
    node.objects.using(using).filter(localization_only=True).delete()


class Migration(migrations.Migration):
    dependencies = [('botend', '0219_simplify_class_guide_articles')]

    operations = [
        migrations.RunPython(ensure_idle, migrations.RunPython.noop),
        migrations.AddField(
            model_name='wowtalentnodemetadata',
            name='localization_evidence',
            field=models.CharField(blank=True, default='', max_length=1000, verbose_name='名称核对依据'),
        ),
        migrations.AddField(
            model_name='wowtalentnodemetadata',
            name='localization_only',
            field=models.BooleanField(default=False, verbose_name='仅名称资料'),
        ),
        migrations.AddField(
            model_name='wowtalentnodemetadata',
            name='name_kind',
            field=models.CharField(choices=[('talent', '天赋'), ('spell', '技能'), ('item', '物品'), ('phrase', '专有名词'), ('macro', '宏名称')], default='talent', max_length=16, verbose_name='名称类型'),
        ),
        migrations.AddField(
            model_name='wowtalentnodemetadata',
            name='reference_aliases',
            field=models.JSONField(blank=True, default=list, verbose_name='来源编号别名'),
        ),
        migrations.AddField(
            model_name='wowtalentnodemetadata',
            name='reference_id',
            field=models.PositiveBigIntegerField(blank=True, null=True, verbose_name='名称引用编号'),
        ),
        migrations.AddConstraint(
            model_name='wowtalentnodemetadata',
            constraint=models.UniqueConstraint(fields=('talent_version', 'name_kind', 'reference_id'), name='wow_name_reference_unique'),
        ),
        migrations.AlterModelOptions(name='wowtalentnodemetadata', options={'verbose_name':'WoW天赋与名称元数据', 'verbose_name_plural':'WoW天赋与名称元数据'}),
        migrations.RunPython(move_names, restore_names),
        migrations.DeleteModel(name='ClassGuideTerm'),
    ]
