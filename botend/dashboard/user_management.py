...
USER_FIELDS = {
    ...
    'is_active', 'is_staff', 'is_superuser', 'password', 'user_group_ids',
}
...
def _user_groups_from_payload(payload):
...
def _serialize_user(user):
    return {
        ...
        'user_groups': [
            ...
            for group in user.dashboard_user_groups.all().order_by('name', 'pk')
        ],
...
class DashboardUserListAPIView(View):
    ...
    def post(self, request):
        ...
        try:
            ...
            groups = _user_groups_from_payload(payload) if not quick_create else []
    ...
    def patch(self, request, user_id):
        ...
        try:
            ...
            with transaction.atomic():
                ...
                groups = None
                if 'user_group_ids' in payload:
                    groups = _user_groups_from_payload(payload)
                ...
                for field in USER_FIELDS - {'password', 'user_group_ids'}:
...
class DashboardUserGroupListAPIView(View):
    def get(self, request):
        ...
        if denied:
            return denied
        groups = DashboardUserGroup.objects.order_by('name', 'pk')
        return JsonResponse({
            'status': 'success',
...
class DashboardUserGroupDetailAPIView(View):
    def patch(self, request, group_id):
        ...
        try:
            payload = _parse_payload(request, allowed_fields=GROUP_FIELDS)
            with transaction.atomic():
                group = get_object_or_404(DashboardUserGroup.objects.select_for_update(), pk=group_id)
                if 'name' in payload:
