...
'''
    ...
    def test_selected_tasks_expose_and_render_actual_frozen_input_differences(self):
        ...
        self.assertIn('function buildCastTimelineModel(tasks)', compare_template)
        self.assertIn('data-timeline-mode="diff"', compare_template)
        self.assertIn('仅看差异', compare_template)
        self.assertIn('timelineStatusLabel', compare_template)
        self.assertIn("status: 'missing'", compare_template)
        self.assertIn("status: 'replacement'", compare_template)
        self.assertIn('castTimelineState.visibleTaskIds', compare_template)
        self.assertIn('cast-timeline-inspector', compare_template)
        self.assertIn('min="1" max="4" value="1" step="0.25"', compare_template)
        self.assertIn('const canvasWidth = Math.ceil(baseCanvasWidth * zoomScale)', compare_template)
        self.assertIn('previousViewportCenter * scroll.scrollWidth', compare_template)
        self.assertIn('baseline-column', compare_template)
        self.assertIn('config-differences-only', compare_template)
        self.assertIn('ability-differences-only', compare_template)
