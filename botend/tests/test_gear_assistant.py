...
class GearAssistantFrontendContractTests(TestCase):
    def test_frontend_assets_expose_owned_and_three_plan_flow(self):
        ...
        for value in ('assistant-current-pane', 'assistant-owned-pane', 'assistant-config-view', 'assistant-results-view', '开始模拟', '再次模拟', '调整配置', '全部解锁'):
            self.assertIn(value, template)
        self.assertIn('data-apply-plan', script)
        for value in ('lockedSlots', 'lockedEquipmentPayload', 'data-toggle-slot-lock', 'setWorkbench("results"', 'markResultsStale'):
