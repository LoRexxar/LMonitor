...
class SimcSourceResolutionFrontendContractTests(unittest.TestCase):
    ...
    def test_battlenet_preflight_waits_for_both_realm_and_character(self):
        collector = MAIN.split('function collectSimcPlayerSource', 1)[1].split('\nfunction requireSimcRunReferences', 1)[0]
        resolution = MAIN.split('async function resolveSimcPlayerSource', 1)[1].split('\nasync function', 1)[0]
        self.assertIn('requireComplete = true', collector)
        self.assertIn('if (requireComplete && (!region || !realm || !character))', collector)
        self.assertIn('collectSimcPlayerSource({ requireComplete: false })', resolution)
        ...
        self.assertLess(
            resolution.index("if (!battlenetRealm || !battlenetCharacter)"),
            resolution.index('fetch(url'),
        )
