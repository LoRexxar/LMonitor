...
class SimcBenchmarkExecutionTests(TestCase):
    ...
    def test_legacy_hero_talent_name_is_standardized_only_for_display(self):
        self.profile.name = 'MID2 默认玩家-法术投射者'
        self.profile.save(update_fields=['name'])
        self.talent.name = 'MID2 默认玩家-法术投射者'
        self.talent.save(update_fields=['name'])
        ...
        self.assertEqual(coordinate['labels']['profile'], 'MID2 默认玩家-疾咒师')
        self.assertEqual(coordinate['labels']['hero_talent'], '疾咒师')
        self.assertEqual(
            coordinate['profile_detail']['talents']['name'],
            'MID2 默认玩家-疾咒师',
        )
    ...
    def test_incremental_projection_exposes_profile_detail_for_selected_coordinate(self):
        ...
        self.assertEqual(profile['talents']['build_code'], 'CYEAoonA')
        self.assertIs(profile['is_ptr'], True)
        self.assertEqual(profile['talent_version'], 'ptr')
    ...
    def test_selected_projection_filters_result_query_before_building_coordinate_detail(self):
        ...
        self.assertEqual(
            result['coordinate_options'][0]['talent_key'],
            result['coordinate_options'][0]['profile_key'],
        )
        self.assertEqual(
            result['coordinate_options'][0]['labels']['talent'],
            result['coordinate_options'][0]['labels']['profile'],
        )
        self.assertEqual(
            result['coordinates'][0]['talent_key'],
            result['coordinates'][0]['profile_key'],
        )
        self.assertEqual(
            result['coordinates'][0]['labels']['talent'],
            result['coordinates'][0]['labels']['profile'],
        )
