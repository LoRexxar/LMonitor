...
from botend.services.midnight_trinket_catalog import (
    build_mid1_panel_payload, mid1_matrix_plan, parse_mid1_catalog,
)
from botend.services.simc_benchmark_config import normalize_panel_payload, replace_panel_config


class Command(BaseCommand):
    help = 'Synchronize an audited MID1 trinket benchmark panel.'

    def add_arguments(self, parser):
        parser.add_argument('--fixture', required=True)
        parser.add_argument('--owner-id', type=int, required=True)
        parser.add_argument('--dry-run', action='store_true')
        parser.add_argument('--slug', default='midnight-s1-trinkets')

    def handle(self, *args, **opts):
        try:
            with open(opts['fixture'], encoding='utf-8') as stream:
                catalog = parse_mid1_catalog(json.load(stream))
        except (OSError, ValueError, json.JSONDecodeError) as exc:
            raise CommandError(f'MID1 catalog rejected: {exc}')
        ...
        try:
            payload = build_mid1_panel_payload(catalog, opts['owner_id'], opts['slug'])
            snapshot = normalize_panel_payload(payload, opts['owner_id'], panel=panel)
        except ValidationError as exc:
            raise CommandError(f'MID1 resources rejected: {exc}')
        self.stdout.write(json.dumps(mid1_matrix_plan(snapshot), ensure_ascii=False, sort_keys=True))
        if opts['dry_run']:
        ...
        try:
            ...
            raise CommandError(f'MID1 Panel rejected: {exc}')
        self.stdout.write(self.style.SUCCESS(
            f'upserted panel id={panel.pk} cases={plan["case_count"]} runs={plan["run_count"]}'
        ))
