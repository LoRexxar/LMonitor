...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        targets = []
        for relative in (row.content_html_path, row.class_content_html_path):
        ...
        if len(set(targets)) != 2:
            raise CommandError('Full and class projections must be separate files')
        ...
        with lock_path.open('a+') as lock_file:
            ...
            try:
                ...
                class_text = paths[1].read_text(encoding='utf-8')
                if (full_text.count("<article class='record'") != expected_count
                        or class_text.count("<article class='spell'") != row.class_spell_count):
