#!/usr/bin/env python
"""在线上安全刷新 MythicDungeonTools 6.2.10 数据。"""
...
SOURCE_VERSION_KEY = 'mdt-6-2-9'
TARGET_VERSION_KEY = 'mdt-6-2-10'
SOURCE_TAG = '6.2.10'
SOURCE_COMMIT = 'e214023441c425a6be2ecea33a5ceb0fc3b87d19'
EXPECTED_COUNTS = {
    'dungeons': 16,
    'enemies': 462,
    'spawns': 3068,
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description=(
            '校验并导入 MDT 6.2.10 内置数据包；首次升级会复用 '
            '6.2.9 数据版本主键，保留路线、短链和人工编辑。'
        ),
    )
    parser.add_argument(
        '--settings',
...
def validate_package():
    ...
    if version.get('key') != TARGET_VERSION_KEY:
        raise RuntimeError('数据包版本键不是预期的 6.2.10。')
    if metadata.get('source_tag') != SOURCE_TAG:
        raise RuntimeError('数据包来源标签不是预期的 6.2.10。')
    if metadata.get('source_commit') != SOURCE_COMMIT:
        raise RuntimeError('数据包来源提交与固定 6.2.10 提交不一致。')
    ...
    if not matriarch or matriarch.get('enemy_forces') != 60:
        raise RuntimeError('Potatoad Matriarch 进度不是 6.2.10 修正后的 60。')
    entrance_count = sum(
...
def main():
    ...
    print('dry-run 通过，开始正式导入并激活 6.2.10。')
    call_command(
        'import_mythic_dungeon_data',
        dry_run=False,
        **command_options,
    ...
    print(f'6.2.10 导入完成并通过数据库复核：{actual_counts}')
...
if __name__ == '__main__':
    try:
        ...
        print(f'6.2.10 导入失败：{exc}', file=sys.stderr)
