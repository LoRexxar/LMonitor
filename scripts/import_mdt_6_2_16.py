#!/usr/bin/env python
"""在线上安全刷新 MythicDungeonTools 6.2.16 数据。"""
...
SOURCE_VERSION_KEY = 'mdt-6-2-15'
TARGET_VERSION_KEY = 'mdt-6-2-16'
SOURCE_TAG = '6.2.16'
SOURCE_COMMIT = '952d152a8694bbde8421dc14995522f853515886'
EXPECTED_COUNTS = {
    'dungeons': 16,
    'enemies': 462,
    'spawns': 3068,
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description=(
            '校验并导入 MDT 6.2.16 内置数据包；升级会复用 '
            '6.2.15 数据版本主键，保留路线、短链和人工编辑。'
        ),
    )
    parser.add_argument(
        '--settings',
...
def validate_package():
    ...
    if version.get('key') != TARGET_VERSION_KEY:
        raise RuntimeError('数据包版本键不是预期的 6.2.16。')
    if metadata.get('source_tag') != SOURCE_TAG:
        raise RuntimeError('数据包来源标签不是预期的 6.2.16。')
    if metadata.get('source_commit') != SOURCE_COMMIT:
        raise RuntimeError('数据包来源提交与固定 6.2.16 提交不一致。')
    ...
    expected_asset_prefix = 'mythic-planner/versions/mdt-6-2-16'
    if asset_snapshot.get('object_prefix') != expected_asset_prefix:
        raise RuntimeError(
            f'静态资源对象前缀不是预期值：{expected_asset_prefix}。'
        )
    ...
    forbidden_asset_paths = {
        ...
        '/vendor/mdt-6.2.5/': '旧 MDT 6.2.5 静态资源路径',
        '/vendor/mdt-6.2.9/': '旧 MDT 6.2.9 静态资源路径',
        '/vendor/mdt-6.2.10/': '旧 MDT 6.2.10 静态资源路径',
        '/vendor/mdt-6.2.11/': '旧 MDT 6.2.11 静态资源路径',
        '/vendor/mdt-6.2.15/': '旧 MDT 6.2.15 静态资源路径',
    }
    ...
    for (dungeon_key, npc_id), expected_spell_ids in released_abilities.items():
        enemy = next(
            (
                row
                for row in dungeon_by_key[dungeon_key].get('enemies') or []
                if row.get('npc_id') == npc_id
            ),
            None,
        )
        actual_spell_ids = {
            int(row['spell_id']) for row in (enemy or {}).get('abilities') or []
        }
        if not expected_spell_ids.issubset(actual_spell_ids):
            missing = sorted(expected_spell_ids - actual_spell_ids)
            raise RuntimeError(
                f'{dungeon_key} NPC {npc_id} 缺少已发布技能：{missing}。'
            )
    if dungeon_by_key.get('temple-of-sethraliss', {}).get(
        'total_enemy_forces'
    ) != 687:
        raise RuntimeError('塞塔里斯神庙总进度不是预期的 687。')
    if dungeon_by_key.get('ruby-life-pools', {}).get('total_enemy_forces') != 551:
        raise RuntimeError('红玉新生法池总进度不是预期的 551。')
    blinding_vale = dungeon_by_key.get('the-blinding-vale') or {}
    if blinding_vale.get('total_enemy_forces') != 686:
        raise RuntimeError('盲目山谷总进度不是预期的 686。')
    matriarch = next(
        (
            enemy
            for enemy in blinding_vale.get('enemies') or []
            if enemy.get('npc_id') == 249756
        ),
        None,
    )
    if not matriarch or matriarch.get('enemy_forces') != 60:
        raise RuntimeError('Potatoad Matriarch 进度不是预期的 60。')
    altar_of_fangs = dungeon_by_key.get('altar-of-fangs') or {}
    twinfang_harrower = next(
        (
            enemy
            for enemy in altar_of_fangs.get('enemies') or []
            if enemy.get('npc_id') == 261554
        ),
        None,
    )
    harrower_clones = {
        int((spawn.get('metadata') or {}).get('source_clone_index') or 0)
        for spawn in (twinfang_harrower or {}).get('spawns') or []
    }
    if not {5, 6}.issubset(harrower_clones):
        raise RuntimeError('尖牙祭坛缺少 6.2.15 新增的双牙收割者刷新点。')
    radiant_spellsower = next(
        (
            enemy
            for enemy in blinding_vale.get('enemies') or []
            if enemy.get('npc_id') == 245336
        ),
        None,
    )
    spellsower_clones = {
        int((spawn.get('metadata') or {}).get('source_clone_index') or 0)
        for spawn in (radiant_spellsower or {}).get('spawns') or []
    }
    if 12 in spellsower_clones:
        raise RuntimeError('盲目山谷仍包含 6.2.15 已移除的多余刷新点。')
    lightfeather = next(
        (
            enemy
            for enemy in blinding_vale.get('enemies') or []
            if enemy.get('npc_id') == 245484
        ),
        None,
    )
    lightfeather_groups = {
        int((spawn.get('metadata') or {}).get('source_clone_index') or 0): (
            spawn.get('group_key') or ''
        )
        for spawn in (lightfeather or {}).get('spawns') or []
    }
    if any(lightfeather_groups.get(index) for index in (5, 6, 7)):
        raise RuntimeError('盲目山谷 6.2.15 的独立花翼刷新点仍被错误编队。')
    lightgorged_lasher = next(
        (
            enemy
            for enemy in blinding_vale.get('enemies') or []
            if enemy.get('npc_id') == 245345
        ),
        None,
    )
    lightgorged_spawn = next(
        (
            spawn
            for spawn in (lightgorged_lasher or {}).get('spawns') or []
            if int((spawn.get('metadata') or {}).get('source_clone_index') or 0) == 1
        ),
        None,
    )
    if not lightgorged_spawn or lightgorged_spawn.get('group_key') != 'group-47':
        raise RuntimeError('盲目山谷缺少 6.2.16 新增的噬光鞭笞者刷新点。')
    lasher = next(
        (
            enemy
            for enemy in blinding_vale.get('enemies') or []
            if enemy.get('npc_id') == 245410
        ),
        None,
    )
    corrected_spawn = next(
        (
            spawn
            for spawn in (lasher or {}).get('spawns') or []
            if int((spawn.get('metadata') or {}).get('source_clone_index') or 0) == 111
        ),
        None,
    )
    corrected_metadata = (corrected_spawn or {}).get('metadata') or {}
    if (
        corrected_metadata.get('source_x') != 447.68273909951
        or corrected_metadata.get('source_y') != -209.5892225802
    ):
        raise RuntimeError('盲目山谷 6.2.16 的鞭笞者位置修正未写入数据包。')
    entrance_count = sum(
...
def main():
    ...
    print('dry-run 通过，开始正式导入并激活 6.2.16。')
    call_command(
        'import_mythic_dungeon_data',
        dry_run=False,
        **command_options,
    ...
    print(f'6.2.16 导入完成并通过数据库复核：{actual_counts}')
...
if __name__ == '__main__':
    try:
        ...
        print(f'6.2.16 导入失败：{exc}', file=sys.stderr)
