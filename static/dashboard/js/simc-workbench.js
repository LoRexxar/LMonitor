/* SimC 十模型内联工作台：专用 API、事件委托和安全结果预览。version: 20260803a */
(() => {
    'use strict';
        ...
        host.innerHTML = data.data.length ? `<div class="mb-3 flex flex-wrap items-center justify-between gap-2 rounded-lg border border-violet-200 bg-violet-50 p-3" data-task-compare-toolbar><span class="text-sm text-violet-900"><i class="fas fa-check-square mr-1" aria-hidden="true"></i>勾选至少两个已完成结果，生成对比页面</span><button type="button" data-task-compare-submit disabled class="simc-touch-action rounded-lg bg-violet-700 px-3 py-2 text-sm text-white disabled:cursor-not-allowed disabled:opacity-50"><i class="fas fa-balance-scale mr-1" aria-hidden="true"></i>生成对比结果（0）</button></div><div class="simc-task-list">${data.data.map(row => {
            if (row.row_type === 'benchmark_execution') {
                ...
                const executionActive = row.is_active === true
                    || (row.is_active == null && ['pending', 'running'].includes(row.status));
                ...
                const totalRunCount = Math.max(0, Number(row.run_count) || 0);
                const runSummaryParts = [
                    `Run ${terminalRunCount}/${totalRunCount}`,
                    `成功 ${runCount('success')}`,
                ];
                if (runCount('running')) runSummaryParts.push(`${executionActive ? '运行' : '中断遗留'} ${runCount('running')}`);
                if (runCount('pending')) runSummaryParts.push(`${executionActive ? '等待' : '未执行遗留'} ${runCount('pending')}`);
                if (runCount('failed')) runSummaryParts.push(`失败 ${runCount('failed')}`);
                if (runCount('cancelled')) runSummaryParts.push(`取消 ${runCount('cancelled')}`);
                const runSummary = runSummaryParts.join(' · ');
                const residueNotice = !executionActive && (runCount('pending') > 0 || runCount('running') > 0)
                    ? '<div class="simc-benchmark-task__residue-note">Execution 已结束；遗留 Run 不会继续自动运行。</div>'
                    : '';
                const executionProgress = row.progress !== null && row.progress !== '' && Number.isFinite(Number(row.progress))
                    ? Math.max(0, Math.min(100, Number(row.progress))) : null;
                const executionProgressLabel = executionActive ? '总进度' : '子任务收口';
                ...
                const summaryProgress = executionProgress == null
                    ...
                    : `<div class="simc-benchmark-task-summary__progress"><span class="simc-benchmark-task-summary__label">${executionProgressLabel}</span><div class="simc-task-progress"><div class="simc-task-progress__track"><div class="simc-task-progress__fill" style="width:${executionProgress}%"></div></div><strong>${executionProgress}%</strong></div></div>`;
                return `<article class="simc-task-card simc-benchmark-task-group"><div class="simc-benchmark-task-summary"><div class="simc-benchmark-task-summary__identity"><div class="simc-task-card__eyebrow"><span class="simc-task-type">基准面板</span><span class="simc-task-id">执行 #${executionId}</span></div><h4 class="simc-task-card__title">${esc(row.name)}</h4></div><div class="simc-benchmark-task-summary__metrics"><div class="simc-benchmark-task-summary__fact"><span class="simc-benchmark-task-summary__label">执行状态</span><span class="simc-task-status is-${esc(row.status)}">${esc(row.status_label)}</span></div><div class="simc-benchmark-task-summary__fact simc-benchmark-task-summary__runs"><span class="simc-benchmark-task-summary__label">Run 状态</span><span>${esc(runSummary)}</span></div>${summaryProgress}</div><button type="button" data-benchmark-task-toggle="${executionId}" aria-expanded="${expanded}" aria-controls="simc-benchmark-task-cases-${executionId}" class="simc-touch-action simc-benchmark-task-summary__toggle">${expanded ? '收起子任务进度' : '展开子任务进度'}</button></div>${residueNotice}<div id="simc-benchmark-task-cases-${executionId}" class="simc-benchmark-task-cases" data-benchmark-task-cases="${executionId}"${expanded ? '' : ' hidden'}>${cases || '暂无独立任务'}</div></article>`;
            }
    ...
    async function showTaskDetail(resource, id) {
        ...
        const artifactList = artifacts.length ? artifacts.map(artifact => `<div class="mt-2 text-sm">${esc(artifact.file_name || artifact.artifact_type || '产物')}${artifact.can_preview === true ? ` <a href="${esc(artifact.preview_url)}" target="_blank" rel="noopener noreferrer" class="text-blue-700">打开报告</a>` : ''}</div>`).join('') : `<p class="mt-2 text-sm text-gray-500">${taskFailed ? '本次失败未生成原生报告' : '暂无结果产物'}</p>`;
        ...
        const resultSummary = report
            ...
            : `<section class="mt-4 rounded-lg border p-4 text-sm text-gray-500">${taskFailed ? '任务失败，暂无可展示的模拟结论。' : '暂无可用的结构化模拟结论。'}</section>`;
        const completeReport = report ? window.SimcResultReport.renderDetails(report) : '';
        ...
        host.innerHTML = `<div class="flex flex-wrap justify-between gap-2"><h4 class="font-bold">任务详情：${esc(row.name || `#${id}`)}</h4><div>${compareButton}${rerunButton}<button class="ml-2" data-wb-close-detail>关闭</button></div></div><dl class="mt-3 grid gap-2 text-sm md:grid-cols-3"><div>状态：${esc(row.status_label)}</div><div>DPS：${esc(row.result_summary?.dps ?? '-')}</div><div>更新时间：${esc(row.updated_at)}</div></dl>${taskErrorPanel}${resultSummary}${comparisonSections}<section class="mt-6 border-t pt-4"><div><h5 class="font-semibold">配置与复现</h5><p class="text-xs text-gray-500">模拟输入、冻结资源、执行轮次与产物集中后置。</p></div>${editable ? `<details class="mt-3 rounded-lg border bg-gray-50 p-3 text-sm"><summary class="font-medium">查看冻结资源与模拟参数</summary><div class="mt-3"><div>Profile #${esc(row.profile_id)} · v${esc(row.profile_version_id)}</div><div>模板 #${esc(row.template_id)} · v${esc(row.template_version_id)}</div><div>APL #${esc(row.apl_id)} · v${esc(row.apl_version_id)}</div><pre class="mt-2 overflow-auto text-xs">${esc(JSON.stringify(params, null, 2))}</pre></div></details>` : ''}<section class="mt-4"><h6 class="font-semibold">执行轮次</h6>${runList}</section><section class="mt-4"><h6 class="font-semibold">结果产物</h6>${artifactList}</section></section>${completeReport}`;
    }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        document.addEventListener('click', async event => {
            ...
            const taskCardAction = event.target.closest('[data-task-card-action]');
            if (taskCardAction && !event.target.closest('a, button, input, label')) {
                const taskId = idOf(taskCardAction.dataset.taskCardId);
                if (!taskId) return;
                if (taskCardAction.dataset.taskCardAction === 'compare') {
                    const checkbox = taskCardAction.querySelector('[data-task-compare-id]');
                    if (checkbox) checkbox.click();
                } else if (taskCardAction.dataset.taskCardAction === 'error') {
                    showTaskDetail('tasks', taskId).catch(notify);
                }
                return;
            }
