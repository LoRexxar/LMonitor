...
(() => {
    ...
    async function showTaskDetail(resource, id) {
        ...
        if (resource === 'batches') {
            ...
            const memberList = members.length ? members.map(member => `<article class="mt-2 flex flex-wrap items-center justify-between gap-2 rounded-lg border p-3"><div><b>${esc(member.name || `任务 #${member.id}`)}</b><div class="text-xs text-gray-500">${esc(member.status_label || member.status)} · ${esc(member.updated_at)}</div></div><button type="button" data-wb-action="detail" data-resource="tasks" data-id="${idOf(member.id)}" class="text-blue-700">查看结果</button></article>`).join('') : empty('此批次暂无成员');
