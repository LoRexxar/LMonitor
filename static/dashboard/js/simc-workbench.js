        ...
        host.innerHTML = data.data.length ? `<div class="mb-3 flex flex-wrap items-center justify-between gap-2 rounded-lg border border-violet-200 bg-violet-50 p-3" data-task-compare-toolbar><span class="text-sm text-violet-900"><i class="fas fa-check-square mr-1" aria-hidden="true"></i>勾选至少两个已完成结果，生成对比页面</span><button type="button" data-task-compare-submit disabled class="simc-touch-action rounded-lg bg-violet-700 px-3 py-2 text-sm text-white disabled:cursor-not-allowed disabled:opacity-50"><i class="fas fa-balance-scale mr-1" aria-hidden="true"></i>生成对比结果（0）</button></div><div class="simc-task-list">${data.data.map(row => {
            if (row.row_type === 'benchmark_execution') {
                ...
                const cases = (row.cases || []).map(item => {
                    ...
                    return `<div class="simc-benchmark-task-case" data-status="${itemStatus}"><div class="simc-benchmark-task-case__header"><span class="simc-benchmark-task-case__title">${title}</span><span class="simc-task-status is-${itemStatus}">${esc(item.status_label || '未知')}</span></div>${itemProgress == null ? '<div class="simc-benchmark-task-case__progress is-unknown">待上报</div>' : `<div class="simc-benchmark-task-case__progress"><div class="simc-task-progress__track"><div class="simc-task-progress__fill" style="width:${itemProgress}%"></div></div><strong>${itemProgress}%</strong></div>`}${error}</div>`;
                }).join('');
                return `<article class="simc-task-card simc-benchmark-task-group"><div class="simc-benchmark-task-summary"><div class="simc-task-card__main"><div class="simc-task-card__eyebrow"><span class="simc-task-type">基准面板</span><span class="simc-task-id">执行 #${executionId}</span></div><h4 class="simc-task-card__title">${esc(row.name)}</h4><div class="simc-task-card__meta"><span class="simc-task-status is-${esc(row.status)}">${esc(row.status_label)}</span><span>${esc(runSummary)}</span></div>${residueNotice}${row.progress == null ? '' : `<div class="simc-task-progress"><div class="simc-task-progress__track"><div class="simc-task-progress__fill" style="width:${row.progress}%"></div></div><span>${executionActive ? '总进度' : '子任务收口'} ${row.progress}%</span></div>`}</div><button type="button" data-benchmark-task-toggle="${executionId}" aria-expanded="${expanded}" aria-controls="simc-benchmark-task-cases-${executionId}" class="simc-touch-action">${expanded ? '收起子任务进度' : '展开子任务进度'}</button></div><div id="simc-benchmark-task-cases-${executionId}" class="simc-benchmark-task-cases" data-benchmark-task-cases="${executionId}"${expanded ? '' : ' hidden'}>${cases || '暂无独立任务'}</div></article>`;
            }
