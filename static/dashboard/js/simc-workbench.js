    ...
    async function loadTasks(page = 1, { background = false } = {}) {
        ...
        host.innerHTML = data.data.length ? `<div class="simc-task-list">${data.data.map(row => {
            if (row.row_type === 'benchmark_execution') {
                ...
                const statusSummary = `本次子任务：已终态 ${terminalCaseCount} / ${Number(row.case_count) || 0} · 成功 ${count('success')} · 部分完成 ${count('partial')} · 失败 ${count('failed')} · 已取消 ${count('cancelled')} · ${executionActive ? '运行中' : '未收口遗留'} ${count('running')} · ${executionActive ? '待运行' : '未调度遗留'} ${count('pending')}`;
                const runSummary = `本次候选 Run：已终态 ${terminalRunCount} / ${Number(row.run_count) || 0} · 成功 ${runCount('success')} · 失败 ${runCount('failed')} · 已取消 ${runCount('cancelled')} · ${executionActive ? '运行中' : '中断遗留'} ${runCount('running')} · ${executionActive ? '待运行' : '未执行遗留'} ${runCount('pending')}`;
                const residueNotice = !executionActive && (runCount('pending') > 0 || runCount('running') > 0)
                    ? '<div class="simc-benchmark-task__residue-note">Execution 已结束；未执行遗留与中断遗留 Run 不会继续自动运行。</div>'
                    : '';
