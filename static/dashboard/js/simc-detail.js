(() => {
  ...
  function renderTask(row) {
    ...
    const abilityRows = abilities.map(item => { const share = percentNumber(item.dps_percent); return `<tr><td class="ability-name">${value(item.name)}</td><td class="right">${number(item.dps)}</td><td class="ability-share"><b>${value(item.dps_percent)}</b><div class="share-track" aria-hidden="true"><div class="share-fill" style="width:${share}%"></div></div></td></tr>`; }).join('');
    ...
    root.innerHTML = `<section class="hero"><span class="pill">任务${statusClass(row)}</span><h1>${value(row.name, `任务 #${objectId}`)}</h1><div class="hero-meta">${characterPills}<span class="pill">更新 ${value(row.updated_at)}</span></div>${nativeReportAction}</section>
      ...
      <div class="grid">
        ...
        ${card('主要技能构成', `<p class="muted">按伤害占比排序；完整施法、暴击、命中与时间线请进入原生报告。</p><div class="table-scroll"><table><thead><tr><th>技能</th><th class="right">DPS</th><th>伤害占比</th></tr></thead><tbody>${abilityRows || '<tr><td colspan="3" class="empty">暂无已解析技能</td></tr>'}</tbody></table></div>`, true)}
