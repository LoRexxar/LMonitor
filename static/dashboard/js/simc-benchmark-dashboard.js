...
function addSpec(data={}){
  ...
  const advanced=advancedGroup('进阶模拟选项','Profile、APL、Template 与 Backend'),grid=el('div',{class:'config-card-grid'});
...
function selectedProfiles(card){const profileId=Number(card.querySelector('[name="profile_id"]')?.value),profile=(resources?.resources?.profiles||[]).find(item=>Number(item.id)===profileId);if(!profileId||!profile)return [];return $$('[data-talent-included]:checked',card).map((included,i)=>{if(!included.value)return null;const talentId=Number(included.value),talent=(resources?.resources?.talent_strings||[]).find(item=>Number(item.id)===talentId);if(!talent)return null;return {profile_id:profileId,talent_string_id:talentId,label:`${profile.name} · ${talent.name}`,is_enabled:true,display_order:i};}).filter(Boolean);}
