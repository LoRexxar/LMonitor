...
const STRUCTURED_PARAMS = ['iterations','target_error','fight_style','max_time','vary_combat_length','enemy_type','desired_targets','enemy_initial_health_percentage'];
const SCENARIO_PRIMARY_PARAMS = ['desired_targets','enemy_initial_health_percentage','max_time','iterations','fight_style'];
...
const SCENARIO_PARAM_VIEW = {
  ...
  enemy_initial_health_percentage:{label:'目标初始生命值 (%)',type:'number',step:'1',min:'1',max:'100',placeholder:'默认满血 100'},
...
function addSpec(data={}){
  ...
  const availableProfiles=(resources?.resources?.profiles||[]).filter(row=>resourceMatches(row,data.spec_key)),availableTalents=(resources?.resources?.talent_strings||[]).filter(row=>resourceMatches(row,data.spec_key));
  const savedProfiles=data.profiles||[],selectedProfile=savedProfiles.find(profile=>profile.is_enabled!==false)||savedProfiles[0]||null;
  const selectedTalents=savedProfiles.filter(profile=>String(profile.profile_id)===String(selectedProfile?.profile_id)&&profile.is_enabled!==false&&profile.talent_string_id!=null).map(profile=>profile.talent_string_id);
  const active=Boolean(persistable&&data.is_enabled!==false&&selectedProfile&&selectedTalents.length);
...
function updateSpecResources(card,selectedProfileId=null,selectedTalentIds=[],original={}){
  ...
  variantAplEditor.hidden=selected.size===0;
  ...
  const addTalent=(value,name,detail='',buildCode='')=>{
    ...
    const explicitApl=saved?.apl?.inherited?'':(saved?.apl?.id??saved?.apl_id??'');
    const input=el('input',{type:'checkbox',value,checked:selected.has(String(value)),dataset:{talentIncluded:String(value)}}),label=el('label',{class:'talent-select-chip'}),copy=el('span',{});
    copy.append(el('strong',{},name));if(detail)copy.append(el('small',{},detail));label.append(input,copy);
...
function renderFourPieceOverride(simulationParams={}){
  const option=(resources?.extra_options||[]).find(item=>item.value==='force_current_tier_4pc');
  const selected=Array.isArray(simulationParams.extra_options)&&simulationParams.extra_options.includes('force_current_tier_4pc');
  const label=el('label',{class:'raid-buff-toggle',title:option?.description||''});
  label.append(el('input',{type:'checkbox',checked:selected,dataset:{forceCurrentTier4pc:''}}),el('span',{},option?.label||'显式覆盖激活四件套'));
  return label;
}
...
function addScenario(data={}){
  ...
  const scenarioParams={enemy_initial_health_percentage:100,...(data.simulation_params||{})};
...
function collectComparisonConfig(){
  ...
  const simulation_params={};STRUCTURED_PARAMS.forEach(key=>{const raw=card.querySelector(`[name="${key}"]`).value.trim();if(raw!=='')simulation_params[key]=['iterations','desired_targets','enemy_initial_health_percentage'].includes(key)?Number(raw):['target_error','max_time','vary_combat_length'].includes(key)?Number(raw):raw;});
...
function comparisonSummaryData(config={},catalogs={}){
  ...
  [['desired_targets','目标数'],['enemy_initial_health_percentage','目标初始生命值'],['max_time','战斗时间'],['iterations','迭代次数'],['fight_style','战斗类型'],['target_error','目标误差'],['vary_combat_length','战斗时长浮动'],['enemy_type','敌人类型']].forEach(([key,label])=>{if(!has(params,key))return;let value=params[key];if(key==='max_time')value=`${value} 秒`;else if(key==='fight_style')value=catalogLabel(catalogs.fight_styles,value);else if(['target_error','vary_combat_length'].includes(key))value=percent(value);add(label,value);});
...
function renderOptionGainDetails(profileDetail,simulationDetail,audit,coordinate={}){
  ...
  appendOptionGainDetailSection(body,'战斗场景',[["战斗类型",style],["目标数量",simulationDetail.desired_targets],["目标初始生命值",simulationDetail.enemy_initial_health_percentage!==undefined?`${simulationDetail.enemy_initial_health_percentage}%`:'100%'],["模拟时长",simulationDetail.max_time?`${simulationDetail.max_time} 秒`:null],["时长浮动",simulationDetail.vary_combat_length!==undefined?`${Number(simulationDetail.vary_combat_length)*100}%`:null],["敌人类型",simulationDetail.enemy_type],["迭代次数",simulationDetail.iterations],["目标误差",simulationDetail.target_error!==undefined?`${Number(simulationDetail.target_error)*100}%`:null]]);
  ...
  if(equipment.length){ const section=el('section',{class:'benchmark-option-detail-section'}), list=el('div',{class:'benchmark-option-detail-equipment'}); equipment.forEach(item=>{ const name=item?.display_name||item?.name_zh||item?.name||`#${item?.item_id||'—'}`, meta=[item?.item_level?`装等 ${item.item_level}`:'',item?.enchant?.display_name?`附魔：${item.enchant.display_name}`:''].filter(Boolean).join(' · '), tooltip=String(item?.tooltip||item?.display_description||'').trim(), attrs={class:'benchmark-option-detail-equipment-row'}; if(tooltip){attrs['data-wow-item-tooltip']=tooltip;attrs['data-wow-item-tooltip-name']=name;attrs.tabindex='0';} const row=el('div',attrs); row.append(el('span',{class:'benchmark-option-detail-slot'},item?.slot_label||item?.slot||'装备'),el('strong',{},name)); if(meta) row.append(el('small',{},meta)); list.append(row); }); section.append(el('h5',{},`装备 (${equipment.length})`),list); body.append(section); }
...
async function rerunFailedPage(id,button){if(!window.confirm('只复制并重跑该次执行中失败或已取消的子任务？\n\n使用原 Execution 冻结的 APL、Profile、Template 与场景输入；历史任务和结果不会被修改。\n如需使用刚保存的 APL，请返回面板选择“全量重新计算”。'))return;setBusy(button,true);try{const data=await benchmarkFetch(`${API}executions/${id}/rerun-failed/`,{method:'POST',body:'{}'});window.location.assign(executionUrl(data.id));}catch(error){notify(`重跑失败：${error.message}`,'error');}finally{if(button.isConnected)setBusy(button,false);}}
async function rerunCase(id,caseId,button){if(!window.confirm('只重跑这个专精子任务？\n\n使用原 Execution 冻结的 APL、Profile、Template 与场景输入；历史任务和结果不会被修改。\n如需使用刚保存的 APL，请返回面板选择“全量重新计算”。'))return;const token=historyGeneration,panelId=historyPanelId;if(!executionPage&&!historyIsCurrent(token,panelId))return;setBusy(button,true);try{const data=await benchmarkFetch(`${API}executions/${id}/cases/${caseId}/rerun/`,{method:'POST',body:'{}'});if(executionPage){renderExecution(data,$('[data-execution-detail]',root));}else if(historyIsCurrent(token,panelId)){renderExecution(data,$('[data-history-detail]',root));await loadHistory(token,panelId);}notify('专精子任务已作为独立执行重新创建','success');}catch(error){notify(`重跑失败：${error.message}`,'error');}finally{if(button.isConnected)setBusy(button,false);}}
...
function bind(){
 ...
 root.addEventListener('change',e=>{if(e.target.matches('[data-benchmark-status]'))renderList();else if(e.target.closest('[data-benchmark-editor]')){if(e.target.matches('[data-editor-benchmark-type]')){panelMetadata.benchmark_type=e.target.value==='option_gain'?'option_gain':'standard';panelMetadata.comparison_option='';syncBenchmarkMode();updateSummary();return;}const raidCard=e.target.closest('[data-config="scenario"],[data-comparison-config]');if(e.target.matches('[data-raid-buff-toggle]')&&raidCard){setRaidBuffSelection(raidCard,e.target.checked?'all':'clear');return;}if(e.target.matches('[data-raid-buff]')&&raidCard){raidCard.dataset.raidBuffExplicit='1';syncRaidBuffControls(raidCard);}if(e.target.matches('[data-use-class-raid-buff]')&&raidCard)raidCard.dataset.classRaidBuffExplicit='1';if(e.target.matches('[data-comparison-extra-option]'))e.target.closest('[data-comparison-config]').dataset.extraOptionsExplicit='1';if(e.target.matches('[data-talent-included]')){const card=e.target.closest('[data-config="spec"]'),included=$$('[data-talent-included]:checked',card);if(included.length>limits.max_profiles_per_spec){e.target.checked=false;notify(`每个专精最多选择 ${limits.max_profiles_per_spec} 个天赋树`,'warning');}const selected=$$('[data-talent-included]:checked',card),editor=$('[data-variant-apl-editor]',card);$$('[data-variant-apl-row]',editor).forEach(row=>{row.hidden=!selected.some(input=>input.value===row.dataset.variantAplRow);});editor.hidden=selected.length===0;}if(e.target.matches('[data-create-spec]'))syncCreateSpecBulkControls();updateSummary();}});
