...
function addSpec(data={}){
  ...
  const selectedTalents=savedProfiles.filter(profile=>String(profile.profile_id)===String(selectedProfile?.profile_id)&&profile.is_enabled!==false).map(profile=>profile.talent_string_id??null);
  const active=Boolean(persistable&&data.is_enabled!==false&&selectedProfile);
  ...
  const advanced=advancedGroup('进阶选项','APL、Template、Backend 与 Profile'),grid=el('div',{class:'config-card-grid'});
  ...
  advanced.body.append(grid);card.append(primary,advanced.details);box.append(card);
...
function selectedProfiles(card){const profileId=Number(card.querySelector('[name="profile_id"]')?.value),profile=(resources?.resources?.profiles||[]).find(item=>Number(item.id)===profileId);if(!profileId||!profile)return [];return $$('[data-talent-included]:checked',card).map((included,i)=>{const talentId=included.value?Number(included.value):null,talent=(resources?.resources?.talent_strings||[]).find(item=>Number(item.id)===talentId);return {profile_id:profileId,talent_string_id:talentId,label:talent?`${profile.name} · ${talent.name}`:profile.name,is_enabled:true,display_order:i};});}
