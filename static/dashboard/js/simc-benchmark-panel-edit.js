(() => {
  ...
  const fill = data => { panel = data; form.elements.panel_name.value = data.name || ''; form.elements.description.value = data.description || ''; form.elements.is_active.checked = !!data.is_active; form.elements.is_public.checked = !!data.is_public; form.elements.schedule_enabled.checked = !!data.schedule_enabled; form.elements.interval_seconds.value = data.interval_seconds || 86400; form.elements.next_run_at.value = localDate(data.next_run_at); status.textContent = '面板已载入'; save.disabled = false; };
  const payload = () => ({name: form.elements.panel_name.value.trim(), description: form.elements.description.value.trim(), is_active: form.elements.is_active.checked, is_public: form.elements.is_public.checked, schedule_enabled: form.elements.schedule_enabled.checked, interval_seconds: Number(form.elements.interval_seconds.value), next_run_at: form.elements.next_run_at.value ? new Date(form.elements.next_run_at.value).toISOString() : null});
