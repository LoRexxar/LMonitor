...
function loadSimcTalentStrings() {
    ...
    fetch('/api/simc-talent-string/?spec=' + encodeURIComponent(spec), { headers: { 'X-CSRFToken': getCSRFToken() } })
        ...
        list.innerHTML = rows.length ? `<table class="min-w-full text-sm"><thead><tr class="border-b text-left text-xs text-slate-500"><th class="px-3 py-2">名称</th><th class="px-3 py-2">专精</th><th class="px-3 py-2">英雄天赋树</th><th class="px-3 py-2">来源</th><th class="px-3 py-2 text-right">操作</th></tr></thead><tbody>${rows.map(row => { const heroNames = (row.hero_talent_names || []).join('、') || '未解析'; const simulator = row.talent_simulator_url ? `<a href="${escapeHtml(row.talent_simulator_url)}" target="_blank" rel="noopener noreferrer" class="mr-2 text-violet-600">天赋模拟器</a>` : ''; return `<tr class="border-b"><td class="px-3 py-3 font-medium">${escapeHtml(row.name)}</td><td class="px-3 py-3">${escapeHtml(row.spec_label || row.spec)}</td><td class="px-3 py-3 text-slate-600">${escapeHtml(heroNames)}</td><td class="px-3 py-3 text-xs text-slate-500">${row.is_system ? '系统资源' : '个人资源'}</td><td class="px-3 py-3 text-right whitespace-nowrap">${simulator}<button type="button" data-talent-string-action="view" data-talent-string-id="${row.id}" class="mr-2 text-slate-600">查看</button><button type="button" data-talent-string-action="copy-code" data-talent-string-id="${row.id}" class="mr-2 text-blue-600">复制字符串</button><button type="button" data-talent-string-action="copy" data-talent-string-id="${row.id}" class="mr-2 text-slate-600">复制资源</button>${row.can_edit ? `<button type="button" data-talent-string-action="edit" data-talent-string-id="${row.id}" class="mr-2 text-blue-600">编辑</button>` : ''}${row.can_delete ? `<button type="button" data-talent-string-action="delete" data-talent-string-id="${row.id}" class="text-red-600">删除</button>` : ''}</td></tr>`; }).join('')}</tbody></table>` : '<p class="py-6 text-center text-sm text-slate-400">暂无天赋字符串</p>';
