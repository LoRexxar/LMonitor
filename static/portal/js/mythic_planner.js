(() => {
    ...
    async function showExportModal(mode = 'export', {shortUrl = '', shortError = '', code = ''} = {}) {
        ...
        openModal({
            ...
            subtitle: sharing ? 'MDT 路线字符串与站内短链接' : 'MythicDungeonTools 6.2.9 路线字符串',
            content: `
                <label>路线分享字符串
                    <textarea id="route-code" readonly>${escapeHtml(shareCode)}</textarea>
                </label>
                <p>使用暴雪 CBOR、Deflate 和 Base64 编码，可直接导入 MythicDungeonTools 6.2.9。</p>
                ${shortUrl ? `
                    <label>站内短链接
        ...
        openModal({
            ...
            content: `
                <div class="mdt-help">
                    ...
                    <div><strong>分享：</strong>可复制 MDT 6.2.9 使用的 <kbd>!~MDT2~</kbd> 暴雪编码路线字符串，也可生成无需登录的站内短链接；两者都包含拉怪组和地图标注。</div>
                    ...
                    <div><strong>数据来源：</strong>副本地图、怪物、刷新点、编队、进度、技能 ID、特性和 POI 直接转换自 <a href="https://github.com/Nnoggie/MythicDungeonTools/tree/6.2.9" target="_blank" rel="noopener noreferrer">MythicDungeonTools 6.2.9</a>，按 GPLv2 保留来源。</div>
